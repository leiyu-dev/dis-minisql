...
- `client <config>`：连接 Coordinator 并在前台运行交互式命令行。
...
flowchart TD
    ...
    Serve --> StartClient[前台 client CLI]
```
...
Coordinator 作为后台 HTTP 服务持续运行。前台 `client` 是普通用户的统一交互入口，它通过 HTTP 连接 Coordinator，支持：

- 直接输入一条 SQL，通过 Coordinator 走分片路由、广播写入或 scatter-gather 查询。
- `shard <key> <sql>`：为读写语句显式指定分片键。
- `nodes`：查看在线 DataNode。
- `metadata`：查看 shard 副本、primary、term 和 commitIndex。
- `health`：查看在线节点和分片汇总。
- `rebalance`：根据当前在线节点重新计算副本布局，并通知节点刷新分片。
- `snapshot`：通知在线 DataNode 创建 snapshot 并压缩 WAL。
- `help`、`quit`/`exit`：查看帮助和退出 client，后台 Coordinator 继续运行。

HTTP API 仍然保留，供自动化测试和脚本调用。Coordinator 运维接口包括：
...
这些接口既用于自动化测试，也方便手工排查。手工使用时推荐通过 `./run.sh` 启动后台 Coordinator 并进入前台 client CLI；退出 client 后后台服务继续运行，需要关闭整套系统时执行 `./stop-system.sh`。
