...
bool DiskManager::IsPageFree(page_id_t logical_page_id) {
  ...
  // Must match DeAllocatePage / AllocatePage extent indexing (BITMAP_SIZE, not PAGE_SIZE).
  size_t bitmap_id = (logical_page_id + BITMAP_SIZE) / BITMAP_SIZE - 1;
  size_t inner_index = logical_page_id % BITMAP_SIZE;
  char *bitmapPage_meta = new char[PAGE_SIZE];
  ReadPhysicalPage(bitmap_id * (BITMAP_SIZE + 1) + 1, bitmapPage_meta);
  auto *bitmapPage = reinterpret_cast<BitmapPage<PAGE_SIZE> *>(bitmapPage_meta);
  bool is_free = bitmapPage->IsPageFree(static_cast<uint32_t>(inner_index));
  delete[] bitmapPage_meta;
  return is_free;
}
...
page_id_t DiskManager::MapPageId(page_id_t logical_page_id) {
  // Logical ids are allocated as extent * BITMAP_SIZE + slot (see AllocatePage).
  page_id_t extent_id = logical_page_id / BITMAP_SIZE;
  page_id_t inner_index = logical_page_id % BITMAP_SIZE;
  return extent_id * (BITMAP_SIZE + 1) + inner_index + 2;
}
