...
2. 已完成：实现 snapshot 和 WAL 压缩。DataNode 会保存 MiniSQL `databases/` 目录快照，并压缩已经包含进 snapshot 的 WAL。
...
5. 最后考虑动态扩容、Raft 强一致和自动化运维能力。

前四项完成后，日志恢复路径已经更清晰，DataNode 恢复成本也不会随 WAL 无限增长；执行器调用也从交互式协议改为更适合服务端调用的 batch 模式；Coordinator 也具备了基础结果解析和跨分片查询后处理能力。后续重点可以转向动态扩容、Raft 强一致和自动化运维。
